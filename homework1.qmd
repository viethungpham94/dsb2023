...
print('Answer: It seems like the seasonal holidays are when there are most flight cancelled (summer holiday, Xmas and New year, and Febuary - Chinese New Year)')
...
print('Answer: It seems unidentified flights are most common, followed by N725MQ')
```
```{r}
#Please `left_join()` the resulting table with the table `planes` (also included in the `nycflights13` package)
merge_plane <- left_join(planes,tailnum_volume_2013)
merge_plane #while this is following the questions to the wording, it doesn't make sense due to year differences?
```
```{r}
#For the plane with the greatest number of flights and that had more than 50 seats, please create a table where it flew to during 2013
#Step 1 - check which plane that is
plane_2013_abv_50_seats <- planes %>% 
                            filter (year == 2013, seats > 50)
plane_2013_abv_50_seats_flight_volume <- left_join(plane_2013_abv_50_seats,tailnum_volume_2013) %>% 
                                            arrange(desc(Count))
plane_2013_abv_50_seats_flight_volume
```
```{r}
#Step 2: The tailnum is N827JB - check where they flew to
flights %>% 
  filter(year == 2013, tailnum == 'N827JB') %>% 
    group_by(dest) %>% 
    summarise(Count=n()) %>% 
    arrange(desc(Count))

print('It seems like # flights to LAX and SFO are the significantly higher')
```
...
ggplot(data = temp_july_2013_count, #use data created
        ...
        geom_point()
print('Answer: The distribution of temp looks vaguely resemblance to a normal distribution')
```
```{r}
...
ggplot(data = wind_speed_july_2013_count, #use data created
        ...
        geom_point()
print('Answer: In terms of outliers, wind speed above 20 have very small counts, can be considered outliers')
```
```{r}
#What is the relationship between `dewp` and `humid`?
ggplot(data=weather, #use overall weather data now
       ...
       geom_point()
print('Answer: It seems there are very little relationship between dewp and humid as the plot show they can be anywhere. A  positive correlation might be observed but variance is high (Rsquare is low)')
```
```{r}
...
ggplot(data=weather_july_2013, #use overall weather data now
       aes(x= dewp, y = humid)) + #plot 2 questioned dataframe
       geom_point()
print('Answer: When looking at just July, it also seems there are very little relationship between dewp and humid as the plot show they can be anywhere. A  positive correlation might be observed (more clearly than overall database) but variance is high (Rsquare is low)')
```
...
ggplot(data=weather, #use overall weather data now
       ...
       geom_point()
print('Answer: It seems there are very little relationship between dewp and humid as the plot show they can be anywhere and no trendline could be observed')
```
